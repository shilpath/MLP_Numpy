...
if __name__ == '__main__':
    ...
    bs = 100
    num_epochs = 50
    w1 = np.random.randn(300, xdata.shape[1])*0.01
    w2 = np.random.randn(100, 300)*0.01
    w3 = np.random.randn(10, 100)*0.01
    b1 = np.random.randn(300, 1)
    ...
    plt.xlabel('epochs')
    plt.ylabel('Accuracy')
    plt.title(f'Backpropagation')
    plt.legend()
    plt.savefig(f'Backpropagation.png', dpi=256)

    Test_DATA_FNAME = 'mnist_testdata.hdf5'
    with h5py.File(Test_DATA_FNAME, 'r') as hf:
        xdata = hf['xdata'][:]
        ydata = hf['ydata'][:]

    a1, a2, a3 = MLP_classifier(xdata, w_list, b_list)

    pred_max_index = np.array([0]*a3.shape[0])
    actual_max_index = np.array([0]*a3.shape[0])
    for i in range(a3.shape[0]):
        pred_max_index[i] = np.argmax(a3[i])
        actual_max_index[i] = np.argmax(ydata[i])
    error = pred_max_index - actual_max_index
    correct_class = 0
    for i in range(len(error)):
        if error[i] == 0:
            correct_class += 1

    print("Accuracy on the test dataset = ", correct_class/a3.shape[0])
